...
/** 선택한 버전의 상세(본문 포함). 미리보기용이라 선택된 항목이 있을 때만 조회한다. */
export function useDocumentHistoryVersion(
  documentId: string | undefined,
  historyId: string | null
) {
  return useQuery({
    queryKey: QUERY_KEYS.documents.historyDetail(documentId ?? '', historyId ?? ''),
    queryFn: () => documentsApi.getHistoryVersion(documentId as string, historyId as string),
