...
변경된 파일들이 FixLog dev-guidelines를 준수하는지 자동으로 검사합니다.

- 프로젝트 구조 규칙 검사 (레이어 역할, 파일 위치, 파일명 규칙)
- Code Fundamentals 17개 규칙 전체 검사
  - 가독성 (01~08): 배타적 경로 분리, 구현 추상화, 로직 분리, 조건 네이밍, 매직 넘버, 시점 이동, 삼항 단순화, 읽기 방향
